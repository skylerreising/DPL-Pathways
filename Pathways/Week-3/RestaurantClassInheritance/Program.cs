...
namespace restaurantAPP
{
  class Program
  {
    ...
    { 
        ...
        // FINE DINING
        // Declare and instantiage a single FineDining object using default constructor
        FineDining exampleRest = new FineDining();

        //Output to show the default constructor values
        Console.WriteLine(exampleRest);

        // Declare and instantiate a single FineDining object using the other constructor
        FineDining anotherExample = new FineDining("Fine Dining Example", "5", 0.2, "Casual", 1);

        // Output to show the default constructor values
        Console.WriteLine(anotherExample);

        //Declare and instantiate the array of FineDining objects
        FineDining[] fineDiningArray = new FineDining[5];

        //Loop and instantiate
        for (int i = 0; i < fineDiningArray.Length; i++)
        {
            fineDiningArray[i] = new FineDining();
        }

        //Load test data
        fineDiningArray[0].RName = "Skyler's Place";
        fineDiningArray[0].RRating = "5";
        fineDiningArray[0].Tip = 0.5;
        fineDiningArray[0].DressCode = "Black Tie";
        fineDiningArray[0].MichelinStars = 3;

        // print each Fine Dining object to test the property gets and the toString

        for (int i = 0; i < fineDiningArray.Length; i++)
        {
            if (!((fineDiningArray[i]).RName == null))
                Console.WriteLine(fineDiningArray[i]);
        }
        
        // Declare and instantiate a single fast food object using the default constructor
        FastFood fastys = new();//simpler declaration
        Console.WriteLine(fastys);
        Console.WriteLine(" ");

        // Declare and instantiate a single FineDining object using the other constructor
        FastFood evenFasters = new("Even Faster's", "4", true, "10AM - 10PM");
        Console.WriteLine(evenFasters);
        Console.WriteLine(" ");

        //Declare and instantiate the array of fast food objects
        var fastFoodArray = new FastFood[5];

        //loop and instantiate
        for(int i=0; i<fastFoodArray.Length; i++)
        {
            fastFoodArray[i] = new FastFood();
            Console.WriteLine(fastFoodArray[i].ToString());
            Console.WriteLine("");
        }

        //load test data
        fastFoodArray[0].RName = "Kelli's Fast Food";
        fastFoodArray[0].RRating = "5";
        fastFoodArray[0].DriveThru = true;
        fastFoodArray[0].HoursOfOperation = "9AM - 8PM";

        Console.WriteLine(fastFoodArray[0]);
        Console.WriteLine("");
